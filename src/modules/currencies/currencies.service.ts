import {
    Injectable, OnModuleInit, Logger, Inject,
...
export class CurrenciesService implements OnModuleInit {
    ...
    private readonly baseUrl = process.env.BOT_API_URL || 'https://gateway.api.bot.or.th/Stat-ExchangeRate/v2/DAILY_AVG_EXG_RATE/';
    ...
    private readonly fallbackCurrencies = [
        { code: 'USD', name: 'สหรัฐอเมริกา : ดอลลาร์ (USD)', buyRate: 32.4313, sellRate: 32.7511 },
        { code: 'GBP', name: 'อังกฤษ : ปอนด์สเตอร์ลิง (GBP)', buyRate: 43.3639, sellRate: 44.1317 },
        { code: 'EUR', name: 'ยูโรโซน : ยูโร (EUR)', buyRate: 37.4028, sellRate: 38.0250 },
        { code: 'JPY', name: 'ญี่ปุ่น : เยน (JPY)', buyRate: 0.2032, sellRate: 0.2090 }, // Normalize แล้ว (ต่อ 1 เยน)
        { code: 'SGD', name: 'สิงคโปร์ : ดอลลาร์ (SGD)', buyRate: 25.2148, sellRate: 25.7528 },
        { code: 'CNY', name: 'จีน : หยวน เรนมินบิ (CNY)', buyRate: 4.6879, sellRate: 4.7755 },
        { code: 'HKD', name: 'ฮ่องกง : ดอลลาร์ (HKD)', buyRate: 4.1304, sellRate: 4.1937 },
        { code: 'AUD', name: 'ออสเตรเลีย : ดอลลาร์ (AUD)', buyRate: 22.7211, sellRate: 23.4431 },
        { code: 'MYR', name: 'มาเลเซีย : ริงกิต (MYR)', buyRate: 8.1808, sellRate: 8.4041 },
    ];
    ...
    async onModuleInit() {
        ...
        if (!apiSuccess) {
            ...
            if (count === 0) {
                this.logger.warn('⚠️ BOT API Offline on startup. Seeding fallback data...');
    ...
    async updateAutoRateAll(): Promise<boolean> {
        try {
            ...
            await this.dataSource.transaction(async (manager) => {
                const repo = manager.getRepository(Currency);
                const rateRes = await firstValueFrom(this.httpService.get(this.baseUrl, {
                ...
                for (const rate of detailRates) {
                    ...
                    // Normalize JPY/IDR
                    if (code === 'JPY') { buy /= 100; sell /= 100; }
                    if (code === 'IDR') { buy /= 1000; sell /= 1000; }
                    ...
                    if (existing) {
                        ...
                        await repo.save(repo.create({
                            code, name: rate.currency_name_th || code,
                            buyRate: buy, sellRate: sell, hasInitialBotData: true
                        }));
                    }
                }
    ...
    async updateManualBulk(updateData: { id: string, buyRate: number, sellRate: number }[]) {
  return await this.dataSource.transaction(async (manager) => {
    const repo = manager.getRepository(Currency);
    ...
    for (const item of updateData) {
      ...
      updatedCodes.push({ code: currency.code, buyRate: item.buyRate, sellRate: item.sellRate });
    }
    ...
    return {
      ...
      note: skippedCodes.length > 0 ? 'Some currencies were skipped because they are in AUTO mode.' : null
    };
  });
}
    // +++++++++++++++++++++++++++ 4. Select All ++++++++++++++++++++++++++++
